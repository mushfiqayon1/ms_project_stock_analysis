...
def main():
    ...
    # print("Acuuracy (SINGLE-layer LSTM)" + str(-acc))
    # #
    ...
    # print ('single_lstm_pred shape', single_lstm_pred.shape) #check
    # #
    ...
    # all_pred['nn_pred'] = np.array(NN.predict(Xtest))
    ...
    print("XtestPoly shape ", XtestPoly.shape)

    all_pred['svm_linear_pred'] = np.array(svm_linear.predict(Xtest))
    all_pred['svm_poly_pred'] = np.array(svm_poly.predict(XtestPoly))
    ...
    for key, pred in all_pred.items():
        ...
        print(roc_auc_score(np.array(Ytest), np.array(pred)))
        print('F1 Score of', key)
        print(eval.f1score(prediction = pred, true_class = Ytest))
        print('recall_score', key)
        # print(eval.recall(prediction = pred, true_class = y_true, average= 'macro'))
        print(eval.recall(prediction = pred, true_class = Ytest))
        print('='*50)
    ...
    for key, pred in all_pred.items():
        portfolio = eval.portfolio_generator(principal = 1000.,prediction = pred, true_price = price,threshold = [0.549,0.55] ,leverage = 1 ,short = True)
